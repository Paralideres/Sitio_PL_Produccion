...
Partial Public Class Lista_LoDestacado
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ''Dim ListadoLoDes As New List(Of InfoLoDestacado)

        ''ListadoLoDes = LoDestacado.ListarLoDestacado(1)

        ''listarTabla(ListadoLoDes)
        ''End If

        Dim Expires As DateTime
        Expires = DateAdd(DateInterval.Minute, intTiempo, Now)
        Dim ListadoLoDes As New List(Of InfoLoDestacado)
        ListadoLoDes = CType(Cache("ListadoDestacado"), List(Of InfoLoDestacado))

        If Cache("ListadoDestacado") Is Nothing Then
            ListadoLoDes = LoDestacado.ListarLoDestacado(1)
            Cache.Insert("ListadoDestacado", ListadoLoDes, Nothing, Expires, System.Web.Caching.Cache.NoSlidingExpiration, System.Web.Caching.CacheItemPriority.Default, Nothing)
        End If

        listarTabla(ListadoLoDes)

    End Sub
    Sub listarTabla(ByVal Listado As List(Of InfoLoDestacado))

        Dim intIndice As Integer = 0
        Dim strTipo As String
        Dim strTitulo As String
        For Each resultado As InfoLoDestacado In Listado
